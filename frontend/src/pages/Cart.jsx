...
const Cart = () => {
  ...
  const handleDelete = (itemId, size) => {
    ...
    setCartData((prevData) => prevData.filter(item => !(item._id === itemId && item.size === size)));
  };
                ...
                <div className='flex items-center gap-6'>
                  ...
                  <div>
                    <p className='text-xs sm:text-lg font-medium'>
                      <b className='sm:text-xl'>{productData.brand}</b>
      ...
      <div className='flex justify-end my-20'>
        <div className='w-full sm:w-[450px]'>
          ...
          <div className='w-full text-end'>
            <button onClick={() => {
              if (getCartAmount() <= 0) {
                toast.error("You didn't purchase anything")
                return;
              }
              else {
